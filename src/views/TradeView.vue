...
<template>
  <section class="page">
    ...
    <ErrorState v-else-if="error" message="二手交易数据加载失败，请检查 Mock 服务是否正常运行。" show-retry @retry="loadTrades" />

    <EmptyState v-else-if="filteredTrades.length === 0" :text="keyword ? '暂无符合条件的搜索结果' : '暂无二手交易信息'" />
      ...
      >
        <template #footer>
          ...
          <template v-if="item.publisher === userStore.displayName">
            <button class="act-btn" @click="$router.push('/publish?edit=trade&id=' + item.id)">编辑</button>
...
.condition { color: var(--text-secondary); font-size: 13px; }
